// script.js — TAF com slider estável (drag real), percurso em canvas e filtro anti-ruído
document.addEventListener('DOMContentLoaded', () => {
  ...
  function restartTimer() {
    ...
    releaseWakeLock();
    stopGpsTracking();
    resetAllSliders(); // garante todos sliders na posição inicial
  }
  ...
  // ===== Sliders (drag estável; exige arrasto real) =====
  ...
  function attachSliderBehavior(slider){
    ...
    const PADDING = 4;
    const HANDLE_W = 48;
    const DRAG_ACTIVATION_PX = 12; // precisa arrastar pelo menos 12px para considerar gesto
    ...
    function onUp(e){
      ...
      const dragAbs = Math.abs(getClientX(e) - startX);

      // Dispara ação somente se arrastou o suficiente e foi até ~90% do trilho
      if (pct >= 0.9 && dragAbs >= DRAG_ACTIVATION_PX) {
